...
function loadStatsPage(){
	...
	var wins = 0;
	var losses = 0;
	for(row_counter = 2; row_counter < table.rows.length; row_counter++){//Outer for loop iterates over each
		cell_value=table.rows[row_counter].cells[2].innerHTML
		cell_value2=table.rows[row_counter].cells[3].innerHTML
		if (cell_value == 7)
			cell_value=07;
		if (cell_value2 == 7)
			cell_value2=07;
		console.log(cell_value, cell_value2);
		if(cell_value>cell_value2){
			table.rows[row_counter].cells[4].innerHTML = "CU Boulder";
			wins++;
		}

		else if(cell_value==cell_value2){
	...
	var table = document.getElementById("wins").innerHTML = wins;
	var table = document.getElementById("losses").innerHTML = losses;
}
